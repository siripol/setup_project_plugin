...
def run(args: argparse.Namespace) -> int:
    ...
    md = renderer.render_markdown(payload, project_encoded, args.since, today)
    ...
    project_human = renderer._human_project(project_encoded)
    out_dir = vault_root / "projects" / project_human / "session-reports"
    ...
    if not args.no_push:
        commit_and_push(vault_root, out_path, stamp, project_human)
...
def commit_and_push(vault_root: Path, out_path: Path, stamp: str, project: str) -> None:
    ...
    No-op (with stderr note) when the vault is not a git repo.
    """
    if not (vault_root / ".git").exists():
        print(
            f"[commit] vault {vault_root} is not a git repo; skipping commit.",
            file=sys.stderr,
        )
        return

    rel = out_path.relative_to(vault_root)
    index_rel = (out_path.parent / "README.md").relative_to(vault_root)
    msg = (
    ...
    try:
        subprocess.run(
            ["git", "-C", str(vault_root), "add", str(rel), str(index_rel)],
            check=True, capture_output=True, text=True,
        )
        subprocess.run(
            ["git", "-C", str(vault_root), "commit", "-m", msg],
            check=True, capture_output=True, text=True,
        )
        subprocess.run(
