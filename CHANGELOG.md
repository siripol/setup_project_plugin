...
- **core-guardrails plugin lifted into marketplace (B3.1 Phase 2, REQ-MKT-002).** First canonical plugin extracted from the scaffold into `platform-marketplace/plugins/core-guardrails/`. Mandatory plugin per spec §6.2 line 233. Ships: `plugin.json` (v1.0.0, mandatory, owners `@siripol/security`), README + CHANGELOG, 9 hook files (audit/chokepoint-gate/rate-limit × sh/py/ts) byte-identical to `templates/claude/hooks/*`, authoritative `settings/settings.patch.json` (permissions.allow + permissions.deny + hooks registrations — REQ-MKT-002 D-8 lock), and two threat-model docs in `rules/` (`deny-sensitive-paths.md` + `deny-network-exfil.md`). Catalog entry appended to `platform-marketplace/.claude-plugin/marketplace.json`. Per ADR-MKT-002 the scaffold's `templates/claude/hooks/*` and the `permissions.{allow,deny}` + `hooks` subtrees of `templates/claude/settings.json` remain unchanged (dual-source; Phase 6 hybrid cutover keeps guardrails template-baked forever). 7 new Phase 2 tests guard the dual-source byte-identity + the consumer-side B2.3 bootstrap hook self-deactivation (silent exit the moment `.claude/plugins/core-guardrails/` exists). 326 passed + 1 skipped → 333 passed + 1 skipped.

- **Platform marketplace producer scaffolding (B3.1 Phase 1, REQ-MKT-002).** Stands up the producer side of the internal plugin marketplace as a new `platform-marketplace/` subdir (ADR-MKT-001 subdir monorepo). Ships `platform-marketplace/.claude-plugin/marketplace.json` (catalog, empty `plugins[]` for now) + `marketplace.schema.json` + `plugin.schema.json` (both JSON Schema Draft 2020-12); `platform-marketplace/{README.md,docs/{PROMOTION.md,VERSIONING.md,RELEASE.md}}` documentation; new `scripts/validate_marketplace.py` CLI with 5 exit codes (0 OK, 1 schema, 2 self-containment, 3 dep-graph, 4 version-sync) and 5 functions (`validate_catalog`, `validate_plugin_manifest`, `validate_self_containment`, `validate_dependency_graph`, `validate_version_sync`); 6 new tests in `tests/test_marketplace_producer.py`; `.github/workflows/ci.yml` extended with a `marketplace-validate` step before pytest; `requirements.txt` pins `jsonschema>=4.18`. No plugin content yet — Phase 2 lifts `core-guardrails` first (closes the B2.3 bootstrap hook self-deactivation loop). Backlog item B3.1 marked `[~]` with branch name. Baseline 320 passed + 1 skipped → 326 passed + 1 skipped.

- **Marketplace divergence warning in `workspace add` (B2.2-FU-4, REQ-WS-002).** `sn-setup workspace add <path>` now reads the new member's `.claude/settings.json` (`installed_plugins[].name`) and `.claude-plugin/marketplace.json` (`marketplace.source`) and compares them against every already-registered member (read live from the filesystem, no registry schema change). Emits severity-tagged stderr warnings: 🔴 critical when `marketplace.source` differs between the new and existing members; 🔴 critical when the new member is missing a mandatory plugin (`core-workflow` / `core-guardrails`) that at least one existing comparable member ships; 🟡 informational when the `installed_plugins` name set has a symmetric difference vs the union of existing members' names (mandatory plugins covered in the red path are excluded from the yellow diff so they aren't double-counted). Findings are warn-only — `workspace add` always succeeds regardless. Legacy members lacking `.claude/settings.json` / `.claude-plugin/marketplace.json` (pre-B2.3 scaffolds, or scaffolds without `--marketplace`) silently skip the compare so they don't generate noise. New helpers `_collect_marketplace_state` + `_check_marketplace_divergence` + a `MARKETPLACE_MANDATORY_NAMES` constant in `scripts/workspace_cli.py`. 8 new tests; baseline 310 passed + 1 skipped → 318 passed + 1 skipped.

- **Internal plugin-marketplace consumer (B2.3, REQ-MKT-001).** New `--marketplace=<source>` flag on `sn-setup new` / `demo` wires the **consumer side** of the org's internal plugin marketplace into every scaffolded service. When set, the scaffold emits `.claude-plugin/marketplace.json` (consumer manifest pointing at the org marketplace source), seeds a top-level `installed_plugins` block in `.claude/settings.json` with the two mandatory plugins (`core-workflow`, `core-guardrails`) plus profile-specific opt-ins (`contracts-sync` + `bff-patterns` for `--profile=bff`) and `compliance-pack` when a regulated policy is planned, and bakes a small SessionStart bootstrap warning hook at `.claude/hooks/marketplace-bootstrap.sh` that nags until `.claude/plugins/core-guardrails/` is present (then self-deactivates). The marketplace catalog itself (which plugins exist + semver pins) lives inside the `core-workflow` (human-facing markdown) and `core-guardrails` (machine-readable manifest + missing-plugin check hook) packs, shipped by **B3.1**; this PR is consumer wiring only. `--marketplace=<source>` accepts a git URL (`https://github.com/...`, `git@github.com:...`, `ssh://...`) or a local path (`./`, `../`, `/abs`, `~`); `org/repo` shorthand is rejected at parse time (platform-ambiguous). Omitting the flag leaves the scaffold unchanged — no `.claude-plugin/` dir, no new settings key, no new hook. New template subtree `skills/sn-setup/templates/marketplace-consumer/default/`. 12 new tests; baseline 297 passed + 1 skipped → 309 passed + 1 skipped. Unblocks `B2.2-FU-4` (marketplace divergence warning in `workspace add`).
